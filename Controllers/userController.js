...
exports.updateUser = async(req, res, next) => {
    ...
    if (req.file) {
        ...
        await User.updateOne({ _id: req.params.id }, { _id: req.params.id, profilePicture: photoCloudinary.url })
...
exports.signup = async(req, res) => {
    ...
    const user = new User({
        ...
        newsLettre: req.body.newsLettre,
        password: hashedPass,
        profilePicture: photoCloudinary.url
    })
    await user.save()
...
exports.signupSocial = (async(req, res) => {
    ...
    if (req.file) {
        const photoCloudinary = await cloudinary.uploader.upload(req.file.path)
        user.profilePicture = photoCloudinary.url

    } else
        user.profilePicture = "https://res.cloudinary.com/showapp/image/upload/v1648755978/nyhzv2togchudd4dgfve.png"
...
exports.NewsLettreAutomatic = (async(req, res, next) => {
    ...
    for (i = 0; i < array1.length; i++) {
        ...
        var mailOptions = { from: process.env.email_app, to: array1[i], subject: 'Account Verification Link', text: 'Hello ' };
    ...
    return res.status(200).json(array1)
...
function templateReset(val, fname) {
    return `
  <!DOCTYPE html >
  <html>
