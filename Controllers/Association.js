...
exports.getAssociationbyid = (req, res, next) => {
    Association.findOne({ _id: req.params.id })
        .then(Association => res.status(200).json(Association))
        .catch(error => res.status(404).json({ message: "Association not found Check id " }));
}

//get all Association
exports.getAllAssociation = (req, res, next) => {
    Association.find()
        .then(Association => res.status(200).json({ associations: Association }))
        .catch(error => res.status(400).json({ error }));
}

//update Association
exports.updateAssociation = async(req, res, next) => {
    await Association.updateOne({ _id: req.params.id }, {...req.body,
            _id: req.params.id
        })
        .then(() => res.status(201).json({ message: 'Association modified !' }))
...
//sign up
exports.signupAssociation = async(req, res) => {

    await Association.init();
    const hashedPass = await bcrypt.hash(req.body.passwordAssociation, 10)
    const association = new Association({
        emailAssociation: req.body.emailAssociation,
        phoneNumberAssociation: req.body.phoneNumberAssociation,
        lastNameAssociation: req.body.lastNameAssociation,
        firstNameAssociation: req.body.firstNameAssociation,
        businessNameAssociation: req.body.businessNameAssociation,
        verifiedAssociation: req.body.verifiedAssociation,
        passwordAssociation: hashedPass
    })

    await association.save()
        .then(() => {

            res.status(201).json({ message: 'Created Association !' })
...
exports.login = (req, res, next) => {
    ...
    Association.findOne({
        ...
        .then(Association => {
            ...
            } else if (Association.verifiedAssociation) {
                bcrypt.compare(req.body.passwordAssociation, Association.passwordAssociation)
                    .then(valid => {
                        if (!valid) {
                        ...
                        res.status(200).json({
                            ...
                            token: jwt.sign({ AssociationId: Association._id },
...
function templateReset(val, name) {
    ...
    <head>
      ...
      <style type="text/css">
        ...
        @tip Set the background color for your email. You may want to choose one that matches your association's branding.
        @theme page
        */
